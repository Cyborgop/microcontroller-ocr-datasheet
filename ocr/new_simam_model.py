...
class SimAM(nn.Module):
    ...
    Computes 3D attention weights from feature statistics using a
    neuroscience-inspired energy function. Zero learnable parameters.
    Fully INT8-compatible (only element-wise ops + sigmoid).
    
    Novel placement: Inside BiFPN fusion nodes (not published elsewhere).
    """
...
class BiFPNModule(nn.Module):
    ...
    def __init__(self, ch_p3, ch_p4, fpn_ch=96):
        ...
        # Novel placement at BiFPN fusion nodes — not in any published work
        self.simam_p3 = SimAM()
        self.simam_p4 = SimAM()
    ...
    def forward(self, p3, p4):
        ...
        p3_td = self.simam_p3(p3_td)  # SimAM attention after P3 fusion
        
        p3_down = self.bu_bn(self.bu_down(p3_td))
        p4_out = self.bu_refine_p4(w_bu[0] * p4_lat + w_bu[1] * p3_down)
        p4_out = self.simam_p4(p4_out)  # SimAM attention after P4 fusion
...
class MCUDetector(nn.Module):
    ...
    - ~0.38M params (8.5× smaller than YOLOv8n)
    ...
    def __init__(self, num_classes):
        ...
        print(
            f"[MCUDetector V3] num_classes={num_classes}, "
...
class MCUDetectionLoss(nn.Module):
    ...
    def _bbox_ciou_batch(pred_boxes, tgt_boxes, inner_ratio=0.7,
        ...
        1. Inner-IoU: Auxiliary scaled boxes (inner_ratio) accelerate
           bbox regression for multi-scale objects.
        2. ShapeIoU: Shape-aware penalty via width/height similarity —
           motivated by MCU boards with distinctive aspect ratios
           (rectangular ICs, square capacitors, elongated resistors).
        3. Focaler: Adaptive linear mapping focuses on hard samples,
           adopted from YOLOv9 (Focaler-IoU, 2024).
        
        Zero additional parameters. Zero inference cost. Same API as CIoU.
        """
        ...
        # ── Step 1: Inner-IoU — compute IoU on auxiliary scaled boxes ──
        ...
        # Standard IoU (for alpha computation)
        ...
        # ── Step 2: ShapeIoU — shape-aware penalty ──
        # Exponential w/h difference penalties for aspect-ratio awareness
        ...
        # ── Combine: Inner-IoU + center + shape penalty ──
        shape_iou = inner_iou - center_dist / c2 - alpha * v - shape_scale * shape_cost

        # ── Step 3: Focaler — adaptive hard-sample focus ──
